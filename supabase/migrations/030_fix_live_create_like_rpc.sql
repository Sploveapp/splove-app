-- Live fix: replace create_like_and_get_result so it never references columns absent on
-- public.conversations (e.g. status, expires_at, updated_at) or public.matches.conversation_id.
-- Inserts: matches (id, user_a, user_b, initiator_user); conversations (id, match_id) only;
-- created_at on conversations comes from table default if present.
...
COMMENT ON FUNCTION public.create_like_and_get_result(uuid) IS
  'Like; réciproque → match (user_a,user_b,initiator_user) + conversation (id,match_id) uniquement.';
